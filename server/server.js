...
app.get('/', (req, res) => { res.json({ message: 'News API Server is running!' }); });
...
app.get('/api/news', (req, res) => {
  ...
  res.json({ status: 'ok', totalResults: filteredArticles.length, articles: filteredArticles });
});

app.get('/api/news/:category', (req, res) => {
  const category = req.params.category;
  const filteredArticles = demoNews.articles.filter(article => article.category.toLowerCase() === category.toLowerCase());
  res.json({ status: 'ok', totalResults: filteredArticles.length, articles: filteredArticles });
});

app.listen(PORT, () => { console.log(`News API server is running on http://localhost:${PORT}`); });
